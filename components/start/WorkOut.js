...
const BeginnerWorkOut = ({
  ...
  //   },
  const {
    translations: { BeginnerWorkOutjs: translated },
  } = useContext(TranslationContext);
  const { workouts } = useWorkoutContext();
  const navigation = useNavigation();
  const handleWithWorkOut = (workOutPresetID) => {
  ...
  return (
    <Animated.View style={{ transform: [{ translateX: fadeAnim }] }}>
      ...
      <ScrollView style={styles.scrollContainer}>
        {Object.values(workouts).map((workoutPreset) => (
          <View key={workoutPreset.id}>
            <TouchableOpacity
              style={styles.workOutContainer}
              onPress={() => handleWithWorkOut(workoutPreset.id)}
            >
              <View style={styles.rowContainer}>
                <Text style={styles.pageDescription}>{workoutPreset.name}</Text>
              </View>
            </TouchableOpacity>
          </View>
        ))}
